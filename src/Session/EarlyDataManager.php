...
class EarlyDataManager
{
     ...
     *
     * @throws InvalidHandshakeDataException 如果早期数据大小超过最大允许值
     */
    public function storeEarlyData(TLS13PSKSession $session, string $data): string
    {
        // 检查早期数据大小是否超过最大允许值
        if ((bool) strlen($data) > $session->getMaxEarlyDataSize()) {
            throw new InvalidHandshakeDataException('Early data size exceeds maximum allowed size');
        }
